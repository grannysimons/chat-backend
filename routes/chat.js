...
router.post("/deleteUser/:idUser", (req, res, next) => {
  ...
  User.findByIdAndDelete(idUser)
    .then(user => {
      let filter = {
        $or: [{ "user1.idUser": idUser }, { "user2.idUser": idUser }]
      };
      Chat.find(filter).then(chats => {
        chats.forEach(chat => {
...
router.post("/deleteMessages/:idUser", (req, res, next) => {
  ...
  Chat.find(filter).then(chats => {
    ...
    return generalError === ""
      ? res.json({ deleteMessages: "ok" })
      : res.json({ deleteMessages: "error deleteMessages: ", error });
  });
});
      ...
      if (user) {
        const filter = {
          $or: [
            {
              $and: [
                { "user1.email": req.session.currentUser.email },
            ...
            },
            {
              $and: [
                ...
                { "user2.email": req.session.currentUser.mail }
              ]
            }
          ]
        };

        Chat.findOne(filter).then(chat => {
          if (!chat) {
            const newChat = Chat({
...
router.post("/typing/:email", (req, res, next) => {
  let email = req.params.email;
  let filter = { $or: [{ "user1.email": email }, { "user2.email": email }] };
  Chat.findOne(filter)
    .then(chat => {
      let idChat = chat._id;
      let idUserDest =
        chat.user1.email === email ? chat.user1.idUser : chat.user2.idUser;
...
router.post("/:email/send", (req, res, next) => {
  ...
  Chat.findOne(filter).then(chat => {
    ...
    return newMessage.save().then(() => {
      Chat.findByIdAndUpdate(chat._id, { dateLastMessage: Date.now() }).then(
        chat => {
          const toUserid =
            chat.user1.idUser == req.session.currentUser._id
              ? chat.user2.idUser
              : chat.user1.idUser;
          const fromUserMail =
            chat.user1.idUser == req.session.currentUser._id
  ...
  Chat.findOne(filter).then(chat => {
    let idInterlocutor =
      chat.user1.idUser == req.session.currentUser._id
        ? chat.user2.idUser
        : chat.user1.idUser;
    User.findById(idInterlocutor).then(user => {
      Message.find({ idChat: chat._id }).then(messages => {
        let filter = {};
        ...
        Chat.findByIdAndUpdate(chat.idChat, filter).then(chats => {
...
router.post("/:idUser/:idChat/totalNewMessages", (req, res, next) => {
  ...
  Chat.findById(idChat)
    .then(chat => {
      const lastSeenUser =
        chat.user1.idUser === idUser
          ? chat.user1.lastSeen
          : chat.user2.lastSeen;
      Message.find({ time: { $gt: lastSeenUser } }).then(messages => {
